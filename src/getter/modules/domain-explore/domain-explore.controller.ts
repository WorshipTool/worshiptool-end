...
import {
    PostSubUrlLoopDto,
    PostAddSubUrlDomainDto,
    PostProcessSubUrlDto
} from "../../getter.dto";
...
export class DomainExploreController {
    constructor(
        ...
        cron.schedule("30 0 */3 * *", () => {
            if (AUTO_GETTER_ACTIVE) this.domainExploreService.checkUpdates();
        });

        cron.schedule("0 2 * * *", () => {
            const time = 60 * 60 * 1000; // 1 hour
            if (AUTO_GETTER_ACTIVE) this.suburlService.processSmartLoop(time);
        });
    }
    ...
    async process(@Body() { url }: PostProcessSubUrlDto) {
        const subUrl = await this.domainExploreSuburlsService.getEntityByUrl(
            url
        );
