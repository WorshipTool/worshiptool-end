...
import {
    ...
    DeletePlaylistQuery,
    GetSongsInPlaylistParams,
    PostAddVariantToPlaylistBody,
    DeleteRemoveVariantFromPlaylistQuery,
    GetIsVariantInPlaylistQuery,
    GetSearchInPlaylistQuery,
    PostReorderPlaylistBody,
    PostTransposePlaylistItemBody
...
import {
    ...
    PostMergeBody,
    PostAddSongDataBody,
    PostVerifyVariantParams,
    PostRenamePlaylistBody,
    GetSongListOfUserResult,
    PostEditVariantBody
} from "./songs.dto";
...
export class SongsController {
     ...
     */
    @ApiOperation({
        summary: "Returns a list of songs by the given search query."
    })
    @AllowNonUser()
    @Get("search")
    ...
    @ApiUnauthorizedResponse({
        description:
            "The user is not authorized to verify the variant. Only admins and trustees can verify variants."
    })
    @ApiBadRequestResponse({
    ...
    async verify(
        @Param() p: PostVerifyVariantParams,
        @User() user: UserObject
    ) {
        if (user.role != ROLES.Admin && user.role != ROLES.Trustee)
            throw new UnauthorizedException(
                "Only admins and trustees can verify variants."
            );
    ...
    @ApiUnauthorizedResponse({
        description:
            "The user is not authorized to unverify the variant. Only admins can unverify variants."
    })
    @ApiBadRequestResponse({
    ...
    async unverify(
        @Param() { guid }: PostVerifyVariantParams,
        @User() user: UserObject
    ) {
        if (user.role != ROLES.Admin)
            throw new UnauthorizedException(
                "Only admins can unverify variants."
            );
    ...
    async createPlaylist(
        @Body() body: PostCreatePlaylistBody,
        @User() user: UserObject
    ) {
    ...
    @ApiTags("Playlists")
    @Delete("playlist")
    async deletePlaylistByGuid(
        @Query() params: DeletePlaylistQuery,
        @User() user: UserObject
    ) {
     ...
     */
    @ApiOperation({
        summary: "Returns a list of variants in the given playlist."
    })
    @ApiUnauthorizedResponse({
    ...
    @ApiUnauthorizedResponse({
        description:
            "The user is not authorized to add the variant to the playlist."
    })
    @ApiNotFoundResponse({
    ...
    @ApiUnauthorizedResponse({
        description:
            "The user is not authorized to remove the variant from the playlist."
    })
    @ApiNotFoundResponse({
    ...
    async removeVariantFromPlaylistDelete(
        @Query() query: DeleteRemoveVariantFromPlaylistQuery,
        @User() user: UserObject
    ) {
        return this.songsService.removeVariantFromPlaylist(
            query.variant,
            query.playlist,
            user
        );
    ...
    async renamePlaylist(
        @Body() body: PostRenamePlaylistBody,
        @User() user: UserObject
    ) {
    ...
    async isVariantInPlaylist(@Query() query: GetIsVariantInPlaylistQuery) {
        return this.songsService.isVariantInPlaylist(
            query.variant,
            query.playlist
        );
    }
    ...
    async searchInPlaylist(
        @Query() params: GetSearchInPlaylistQuery,
        @User() user: UserObject
    ) {
        const result = await this.playlistService.searchInPlaylist(
            params.guid,
            params.searchKey,
            params.page,
            user
    ...
    @ApiBadRequestResponse({
        description:
            "Playlist or items are undefined, out of range or not unique."
    })
    @ApiBearerAuth()
    @Post("playlist/reorder")
    async reorderPlaylist(
        @Body() body: PostReorderPlaylistBody,
        @User() user: UserObject
    ) {
        return await this.playlistService.reorderPlaylist(
            body.guid,
            body.items,
            user
        );
    ...
    @ApiUnauthorizedResponse({
        description:
            "The user is not authorized to transpose the playlist item."
    })
    @ApiNotFoundResponse({
    ...
    async transposePlaylistItem(
        @Body() body: PostTransposePlaylistItemBody,
        @User() user: UserObject
    ) {
        return await this.playlistService.transposePlaylistItem(
            body.guid,
            body.key,
            user
        );
    ...
    async getSongListOfUser(
        @User() user: UserObject
    ): Promise<GetSongListOfUserResult> {
        return {
