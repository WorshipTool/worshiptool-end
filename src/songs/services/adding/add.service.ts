...
export class AddSongDataService {
    ...
    async processNewSongData(
        data: Partial<NewSongData>,
        user: User
    ): Promise<NewSongDataProcessResult> {
        ...
        let song: Song = await this.songRepository.findOne({
            where: { guid: songGuid }
        });
        if (song === null) {
        ...
        if (data.sheetData || data.title || data.source) {
            ...
            if (data.title) {
                ...
                const titleGuid = await (
                    await this.nameRepository.insert(titleData)
                ).identifiers[0].guid;
                title = await this.nameRepository.findOne({
        ...
        if (data.media) {
            for (let i = 0; i < data.media.length; i++) {
                ...
                if (!checkMediaFormat(media)) {
                    console.log(
                        "Media won't be inserted, because of wrong format of url."
                    );
                } else {
                    const ex = await this.mediaRepository.findOne({
                        where: {
                            ...media,
                            song: song
                        }
                    ...
                    if (!ex) {
                        const r = await this.mediaRepository.insert({
                            ...media,
                            song
                        });
                    } else {
        ...
        if (data.tags) {
            for (let i = 0; i < data.tags.length; i++) {
                ...
                const s = await this.songRepository.findOne({
                    where: { guid: songGuid },
                    relations: {
                        tags: true
                    }
                ...
                await this.songRepository.save({
                    ...s,
                    tags: s.tags ? [...s.tags, entity] : [entity]
                });
            }
        }
